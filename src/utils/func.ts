...
export function menutext(client: Client) {
  creatorname();
  const waitwhatyoucando = langat !== "en" ? gradient(["#2180db", "#75a1cb", "#78a5d3"])(["[0] Salut, si vous êtes ici, vous cherchez probablement à savoir comment ces codes fonctionnent ici\n", "[0] C'est assez simple, il suffit d'appliquer le principe d'initialisation de fonction\n", "[0] Amusez-vous bien, code développé par K-M19\n"][Math.floor(Math.random() * 3)]) : null;
  console.log((waitwhatyoucando ? waitwhatyoucando : '') + gradient(["#7fffd4", "#76eec6", "#00ffff"])(t("menuText")));
  choiceinit(client);
}
  ...
  console.log(
    gradient(["#7fffd4", "#76eec6", "#00ffff"])(
      t(`Nome da conta:: ${client.user.username}\nGlobal Account Name: ${client.user.globalName
        ...
        }\nEmail: ${client.user.emailAddress}\nphone number : ${client.user.phoneNumber
  ...
  const proceedWithCloning = async () => {
    try {
      ...
      if (createNewServer) {
        ...
        if (!newGuild) {
          console.error(gradient(["red", "darkred"])('A fatal error occurred while creating the server, the cloner will be restarted in 10 seconds'));
          errors++;
          setTimeout(() => {
            clearall();
          }, 10000);
      ...
      if (!cloner) {
        console.error(gradient(["red", "darkred"])('A fatal error occurred while cloning and the cloner will be restarted in 10 seconds'));
        errors++;
        setTimeout(() => {
          clearall();
        }, 10000);
      ...
      setTimeout(async () => {
        ...
        console.log(gradient(["#a2e4a1", "#a2e4a1", "#a2e4a1", "#a2e4a1"])(t('msgfinalcloner') + Tempo2));
        console.log(gradient(["#a2e4a1", "#a2e4a1", "#a2e4a1", "#a2e4a1"])(t('configtime') + Tempo));
        console.log(gradient(["#a2e4a1", "#a2e4a1", "#a2e4a1", "#a2e4a1"])(t('channelnumber') + clonedall));
        console.log(gradient(["#a2e4a1", "#a2e4a1", "#a2e4a1", "#a2e4a1"])(t('errorcloning') + errors));

        if (cloneOption === 3) {
          const template = await newGuild.createTemplate(
            `${guild.name}`,
            `By K-M19 (https://github.com/K-M19)`
          );
          console.log(gradient(["#a2e4a1", "#a2e4a1", "#a2e4a1", "#a2e4a1"])(`» Template Url: ${template.url}`));
        }
      ...
      console.error('A specific error occurred during cloning: ', error);
      errors++;
      rl.close();
    }
...
export async function serverinfo(client: Client) {
  async function fetchGuildData(guildId: string) {
    try {
      ...
      console.error(
        gradient(["#7fffd4", "#76eec6", "#00ffff"])("An error occurred:"),
        error
      );
    }
    awaitenter(client);
...
async function yop(question: string): Promise<boolean> {
  const answer = await espop(question + gradient(["#a2e4a1", "#a2e4a1", "#a2e4a1", "#a2e4a1"])((t('yandn'))));
  return answer === "1";
}
