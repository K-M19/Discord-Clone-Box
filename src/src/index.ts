...
export const fetch = (backupID: string) => {
  return new Promise<BackupInfos>(async (resolve, reject) => {
    getBackupData(backupID)
      .then((backupData) => {
        const size = statSync(`${cloner}${sep}dumbcloner.json`).size;
...
export const load = async (
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      return reject("Unable to continue cloning: No json found\nYou can try again or report the error. ");
    }
  });
};
...
export const remove = async (backupID: string) => {
  return new Promise<void>((resolve, reject) => {
    try {
      require(`${cloner}${sep}dumbcloner.json`);
      unlinkSync(`${cloner}${sep}dumbcloner.json`);
      resolve();
    } catch (error) {
      reject("Not found");
    }
